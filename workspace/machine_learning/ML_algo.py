...
def neural_network(X, y, algo_name):
	...
	clf = MLPClassifier(activation='tanh', solver='sgd', alpha=0.0001,hidden_layer_sizes=(1000, 500, 300), batch_size=32,
	 learning_rate_init=0.001, max_iter=30)
	# clf = MLPClassifier(hidden_layer_sizes=(1000, 500, 300), max_iter=2000)
...
def svm(X, y, algo_name):
	...
	# print(X_normalized)
	# pd.DataFrame(X_normalized).to_csv("X_normalized.csv", header=None, index=None)
	run_algo(clf, X, y, algo_name)

def k_nearest_neighbor(X, y, algo_name):
	clf = KNeighborsClassifier(n_neighbors=5) #tune DISTANCES
	run_algo(clf, X, y, algo_name)
...
def algorithms_selection(algo_name):
	...
	# file_name = "cnn_dataframe.csv"
	file_name = "cnn_dataframe_29021.csv"
	...
	# X = joined_df.iloc[:,0:5000].values.tolist()
	# X = joined_df.iloc[:,0:29021].values.tolist()
	...
	pd.DataFrame(X).to_csv("cnn_X_data.csv", header=None, index=None)
	pd.DataFrame(y).to_csv("cnn_y_data.csv", header=None, index=None)

	# df_X = pd.read_csv('cnn_X_data.csv')
	# df_y = pd.read_csv('cnn_y_data.csv')
