...
def isolation_forest(file_name, path_to_file_name, is_Simul, isMax, max_number_or_var, min_number):
	...
	# print_comparison("IF", "path_to_file_name", outliers_list, number_of_outliers, df, df_removed, number_of_all_features)
	df_removed = pd.read_csv("/home/batyi/Desktop/Study/Study project/website_fingerprinting_proxy-stable/workspace/outlier_detection/data_for_plots/1_www.google.com_removed.csv")
	print(path_to_file_name)
	print(file_name)
	print("Outliers based on IsolationForest:")
	df = df.drop(columns="index")
	df_cumul_sum = df_cumul_sum.drop(columns="index")
	df_removed = df_removed.drop(columns="index")
	# print(df)
	# print(df_removed)
	print_comparison("IS", path_to_file_name, outliers_list, 9, df, df_removed, len(df_cumul_sum.index) - 1) #print information
...
def upper_bound_check(is_Simul, df_outliers, isMax, max_number_or_var, number_of_outliers, df_removed, outliers_list):
	if(df_outliers.empty == False): #if any outliers exists
		outliers_from_subpage = {}
		for index, i in enumerate(df_outliers.index):
			if(is_Simul == False):
				if(isMax):
...
def measuring_variance(df): #calculate the sum of the mean variance (distance between instances of one page)
	...
	print("number_of_distances: ", number_of_distances)
	...
	for group_index, group_webpage in enumerate(all_lists):
		for webpage_index, webpage in enumerate(group_webpage):
			if(check(webpage, outlier_list)):
				plt.plot(np.arange(0, 50, 1), df[webpage].to_numpy()[0:50] / 1000.0, 'o', label = webpage, color=colors[group_index % len(colors)][webpage_index % 10])
			else:
				plt.plot(np.arange(0, 50, 1), df[webpage].to_numpy()[0:50]  / 1000.0, '-', label = webpage, color=colors[group_index % len(colors)][webpage_index % 10])
...
def simulation_mode(url_of_input_file, output_file):
 	...
 	# isolation_forest(file_name, path_to_file_name, False, False, -1, -1)
...
def invoke_function(algorithm_name, min_number_to_remove, max_number_or_var, isMax, input_file):
	...
	path_to_file_name = SUP.merge_file_path(file_name) 		#call merge_file_path() function to enter into the directory of "file_name"
	print(min_number_to_remove)
	print(max_number_or_var)
...
# Outliers based on IsolationForest:
# [{'1_3_www.google.com': [106, 107]}, {'1_5_www.google.com': [214, 215]}, {'1_6_www.google.com': [267, 268, 269]}, {'2_6_www.google.com': [265, 269]}, {'3_7_www.google.com': [315, 318, 319, 323]}, {'4_3_www.google.com': [106, 107]}, {'4_6_www.google.com': [265, 268, 269]}, {'4_8_www.google.com': [376, 377]}, {'4_9_www.google.com': [430, 431]}]



# Outliers based on IQR based on final CUMUL_SUM:
# [{'index': 54.0}, {'3_2_www.google.com': 749260.0, '3_9_www.google.com': 772612.0}, {'3_10_www.google.com': 2597571.0}]
